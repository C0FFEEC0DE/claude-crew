...
The GitHub smoke workflow default is `6` turns per task (full/nightly use `8` turns) so CI stays bounded for small models; raise it manually in `workflow_dispatch` when you want a slower debug run.
...
**Timeout fallback (400s to 600s):** The full-suite workflow defaults to 600s per task (`behavior-benchmark-full.yml` line 219: `timeout_seconds="${INPUT_TIMEOUT_SECONDS:-${VAR_BENCHMARK_TIMEOUT_SECONDS:-600}}"`). The smoke and subagent smoke workflows default to 400s. Both are overridable via `VAR_BENCHMARK_TIMEOUT_SECONDS` or `workflow_dispatch` input. The runner's `subprocess.run(timeout=CLAUDE_TIMEOUT_SECONDS)` uses the value passed via the `CLAUDE_TIMEOUT_SECONDS` environment variable, which is set from the workflow's resolved `timeout_seconds`.
...
**Note on forbidden patterns:** Keep `forbidden_transcript_patterns` minimal (e.g., `["footer repair", "shell guard"]`) to avoid false positives. The golden and smoke suites use the same forbidden pattern baseline to ensure consistent validation tiers.
