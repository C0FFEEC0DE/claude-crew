...
- `PreToolUse` / `PermissionRequest` / `PermissionDenied` — block dangerous or out-of-scope commands, including force-push, `mkfs*`, and remote bootstrap pipes; `PermissionDenied` only requests a retry for commands that are not hard-blocked by profile policy and the session is not running in CI or benchmark headless mode
